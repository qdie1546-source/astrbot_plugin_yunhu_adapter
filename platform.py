...
class YunHuPlatform(Platform):
    ...
    async def run(self):
        """启动平台（WebSocket 接收，若有）"""
        await self.client.start()
        self._running = True
        # 注册事件处理器（若使用 WebSocket）
        if self.ws_url:
            ...
            logger.info("云湖平台已启动（仅HTTP发送，无WebSocket接收）")
    ...
    async def send_message(self, event: AstrMessageEvent):
        ...
        chat_id = message.session_id  # 根据实际情况确定聊天ID
        # 将 AstrBot 消息链转为云湖消息
    ...
    async def process_webhook(self, data: Dict[str, Any]):
        """
        处理云湖推送的 webhook 消息（在 config_flow.py 中调用）
        根据实际推送格式解析，构造 AstrMessageEvent 并分发
        """
        # 示例：假设云湖推送格式为 { "message": { "chat_id": "...", "text": "...", "sender": {...} } }
        # 请根据官方文档调整
        try:
            ...
            is_group = chat_id != sender_id  # 简单的群聊判断
    ...
    # 以下方法用于 WebSocket 接收，若使用 webhook 可忽略
